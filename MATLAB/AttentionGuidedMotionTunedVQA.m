...
function [q qPF] =  AttentionGuidedMotionTunedVQA(v1, v2, orientationSet)
%% parameter setting
if nargin < 3
   orientationSet = 2;
end
...
w = size(v1,2);
nFrame = size(v1,3);
chunkSize = 2*filter_half_width+1;
nChunk = floor((nFrame-filter_half_width) / (filter_half_width+1));
...
for i = 1:nChannel
    for j = 1:2 % marginalized + unmarginalized
        volSE1{i,j} = zeros(h,w,nChunk);
        volSE1_n{i,j} = zeros(h,w,nChunk);
        volSE2_n{i,j} = zeros(h,w,nChunk);
    end
end

for c = 1 : nChunk
%     fprintf('Processing Chunk %d\n', c);
    chunkStartPos = 1+(c-1)*(filter_half_width+1);
    chunkRange = chunkStartPos:chunkStartPos+chunkSize-1;
    [temp1, temp2] = spacetimeOrientationAnalysis(...
        v1(:,:,chunkRange), allOrientations, ...
        G2H2OrG3, doMarginalize, filter_half_width);
    for i = 1:nChannel
        for j = 1:2
            volSE1{i,j}(:,:,c) = temp1{i,j};
            volSE1_n{i,j}(:,:,c) = temp2{i,j};
        end
    end
    clear temp1 temp2
end
...
%     fprintf('Processing Chunk %d\n', c);
    ...
    for i = 1:nChannel
        for j = 1:2
            volSE2_n{i,j}(:,:,c) = temp2{i,j};
        end
    end
    clear temp2
end
% [~, volSE2_n] = spacetimeOrientationAnalysis(v2, allOrientations, G2H2OrG3, ...
