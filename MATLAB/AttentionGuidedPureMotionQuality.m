...
%% Notes
% The size of filter
% Normalize energy at a small area or not?  (agg_size) -> YES
...
%     fprintf('Processing Chunk %d\n', c);
    ...
    [~, temp2_d] = spacetimeOrientationAnalysis(...
        ...
        G2H2OrG3, doMarginalize, filter_half_width);
    temp_volSI = getSaliencyWeights(temp1_r, temp2_r);
    [~, temp_qPF] = computeSimilarity2(temp2_r, temp2_d, temp_volSI);
    scorePos(1) = scorePos(2) + 1;
    scorePos(2) = scorePos(1) + size(temp_qPF,1)-1;
    scoreRange = scorePos(1):scorePos(2);
    qPF(scoreRange,:) = temp_qPF;
    clear temp1_r temp2_r temp2_d temp_volSI
end
q  = mean(qPF);
end
